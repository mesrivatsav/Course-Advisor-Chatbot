...
def get_plain_text_with_header(content: Soup) -> str:
    # create clean text
    data = " "
    for items in content.find_all('main',class_='main-content-wrapper'):
...
def load_and_split_docs():
    sitemap_loader = SitemapLoader("https://www.tudublin.ie/sitemap-en.xml",
...
#Chroma.from_documents(documents=unique_docs, persist_directory="./chroma_db",ids=unique_ids,collection_name="TUS_COURSES", embedding=embeddings)
#documents = [Document(page_content=doc.page_content, metadata={"TU name":"Technological University of Dublin","TU":"TUD"}) for _,doc in enumerate(unique_docs)]
Chroma.from_documents(documents=unique_docs, persist_directory="../chroma_db",ids=unique_ids,collection_name="TUD", embedding=embeddings)