...
def get_plain_text_with_header(content: Soup) -> str:
    # create clean text
    data = " "
    for items in content.find_all(id="maincontent"):
...
def load_and_split_docs():
    sitemap_loader = SitemapLoader("https://tus.ie/sitemap_index.xml",
...
#documents = [Document(page_content=doc.page_content, metadata={"TU name":"Technological University of Shannon","TU":"TUS"}) for _,doc in enumerate(unique_docs)]
Chroma.from_documents(documents=unique_docs, persist_directory="../chroma_db",ids=unique_ids,collection_name="TUS", embedding=embeddings)